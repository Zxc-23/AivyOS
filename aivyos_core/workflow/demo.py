...
async def run(request: str, resume: bool, cfg: dict) -> None:
    ...
    if resume:
        print("== 从检查点续传 ==")
        state = await graph.resume(thread, ctx={})
    else:
        print("== 首次执行 ==")
        state = await graph.invoke({"user_request": request}, thread_id=thread, ctx={})
...
def main(argv: list[str] | None = None) -> None:
    ...
    asyncio.run(run(args.demo, args.resume, cfg))
